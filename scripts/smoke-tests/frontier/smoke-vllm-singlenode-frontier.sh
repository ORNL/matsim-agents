...
# Ray telemetry and cluster auto-discovery
...
srun -N1 -n1 -c56 --gpus-per-task=${GPUS_PER_NODE} --gpu-bind=closest \
  "$UTILS_DIR/_rocr_to_hip.sh" "$VENV/bin/python" -c "
import torch, time
...
srun -N1 -n1 -c56 --gpus-per-task=${GPUS_PER_NODE} --gpu-bind=closest \
  "$UTILS_DIR/_rocr_to_hip.sh" "$VENV/bin/python" -m vllm.entrypoints.openai.api_server \
...
# ── Wait for /health ─────────────────────────────────────────────────────────
echo "[vllm] Waiting for server (up to 20 min) ..."
MAX_WAIT=1200
ELAPSED=0
INTERVAL=10
while true; do
