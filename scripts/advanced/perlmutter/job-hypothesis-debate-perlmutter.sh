...
#   MATSIM_DEBATE_SMOKE=1 sbatch scripts/advanced/perlmutter/job-hypothesis-debate-perlmutter.sh
#
# Full run:
#   sbatch scripts/advanced/perlmutter/job-hypothesis-debate-perlmutter.sh
# ---------------------------------------------------------------------------
...
for CASE in $CASES; do
  ...
  CUDA_VISIBLE_DEVICES="" python "$REPO/scripts/advanced/perlmutter/hypothesis_debate_beforeafter.py" \
      "${CASE_ARGS[@]}" \
      --runs-root "$RUNS_ROOT" \
      --output-dir "$CASE_OUT" \
      --provider vllm \
