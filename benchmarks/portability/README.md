...
`all_model_scientific_debate.py` is the live portability test for first-class
catalog models whose checkpoints are stored on the facility's parallel
filesystem. It dynamically reads `deployments/common/open-model-catalog.json`
...
assigned materials-design question is:
...
Each catalog entry declares a `base_url_env`. Export the corresponding endpoint
variable for every locally selected model (or one `MATSIM_VLLM_BASE_URL`
endpoint capable of routing all selected model IDs), then run:
...
machine-readable result records nonlocal and policy-disabled catalog entries and fails
for fewer than two local models, a missing selected-model endpoint, missing or
empty turns, duplicate contribution IDs, or empty synthesis.
