#!/usr/bin/env python3
"""Strong-scaling of the AL DFT-labelling stage vs. node allocation.

Reads the per-N scaling runs produced by
``scripts/advanced/perlmutter/submit-al-dft-scaling-sweep.sh`` (RUN_TAG
``hea-fcc-scaling-N<nodes>``), each of which labels an identical fixed workload
(16 VASP single-points) with ``<nodes>``-way concurrency. Plots (a) DFT
throughput (labelled frames per hour) and (b) parallel efficiency vs. node
count, against the ideal-linear reference.

Usage
-----
    fairchem_venv/bin/python plot_dft_scaling.py \
        --runs-root /global/cfs/projectdirs/m5216/mlupopa/runs \
        --out al_dft_scaling.pdf

Run from ``docs/paper/figures``. Also prints the numbers cited in the text.
"""

from __future__ import annotations

import argparse
import glob
import json
import os
import re

import matplotlib

matplotlib.use("Agg")
import matplotlib.pyplot as plt
import numpy as np

TAG_RE = re.compile(r"hea-fcc-scaling-N(\d+)$")


def collect(runs_root: str) -> list[tuple[int, int, float]]:
    """Return sorted [(nodes, n_converged, dft_seconds), ...] over scaling runs."""
    rows: list[tuple[int, int, float]] = []
    for d in glob.glob(os.path.join(runs_root, "hea-fcc-scaling-N*")):
        m = TAG_RE.search(os.path.basename(d))
        if not m:
            continue
        nodes = int(m.group(1))
        states = sorted(glob.glob(os.path.join(d, "iteration_*", "state.json")))
        conv = 0
        dft = 0.0
        for s in states:
            try:
                j = json.load(open(s))
            except Exception:  # noqa: BLE001
                continue
            conv += j.get("n_dft_converged", 0)
            dft += j.get("timings_sec", {}).get("dft", 0.0)
        if conv and dft > 0:
            rows.append((nodes, conv, dft))
    return sorted(rows)


def main() -> None:
    ap = argparse.ArgumentParser()
    ap.add_argument("--runs-root", required=True)
    ap.add_argument("--out", default="al_dft_scaling.pdf")
    args = ap.parse_args()

    rows = collect(args.runs_root)
    if not rows:
        raise SystemExit(
            "No scaling runs found (expected RUN_TAG hea-fcc-scaling-N<nodes>). "
            "Submit scripts/advanced/perlmutter/submit-al-dft-scaling-sweep.sh first."
        )

    nodes = np.array([r[0] for r in rows], dtype=float)
    conv = np.array([r[1] for r in rows], dtype=float)
    dft_h = np.array([r[2] for r in rows], dtype=float) / 3600.0
    throughput = conv / dft_h  # frames per hour

    base = throughput[0]  # 1-node throughput (nodes assumed sorted, first = min)
    base_nodes = nodes[0]
    ideal = base * nodes / base_nodes
    efficiency = 100.0 * (throughput / nodes) / (base / base_nodes)

    fig, (ax_t, ax_e) = plt.subplots(1, 2, figsize=(8.6, 3.3))

    ax_t.plot(nodes, ideal, "k--", lw=1.2, label="ideal (linear)")
    ax_t.plot(nodes, throughput, "o-", color="#3b6fb0", lw=1.6, ms=6, label="measured")
    ax_t.set_xlabel("Nodes (= concurrent VASP jobs)")
    ax_t.set_ylabel("DFT throughput (frames / h)")
    ax_t.set_title("(a) DFT-labelling throughput", fontsize=9)
    ax_t.set_xticks(nodes)
    ax_t.legend(fontsize=8)

    ax_e.axhline(100.0, color="k", ls="--", lw=1.2)
    ax_e.plot(nodes, efficiency, "s-", color="#e08214", lw=1.6, ms=6)
    ax_e.set_xlabel("Nodes (= concurrent VASP jobs)")
    ax_e.set_ylabel("Parallel efficiency (%)")
    ax_e.set_title("(b) Strong-scaling efficiency", fontsize=9)
    ax_e.set_xticks(nodes)
    ax_e.set_ylim(0, 115)
    for xi, e in zip(nodes, efficiency):
        ax_e.text(xi, e, f"{e:.0f}%", ha="center", va="bottom", fontsize=7)

    fig.tight_layout()
    fig.savefig(args.out, bbox_inches="tight")
    print(f"wrote {args.out}")

    print("\nnodes, converged, dft_h, throughput(frames/h), speedup, efficiency%")
    for i in range(len(nodes)):
        print(
            f"  N={int(nodes[i]):2d}  conv={int(conv[i]):3d}  dft_h={dft_h[i]:.3f}  "
            f"tput={throughput[i]:7.1f}  speedup={throughput[i]/base:4.2f}x  "
            f"eff={efficiency[i]:5.1f}%"
        )


if __name__ == "__main__":
    main()
