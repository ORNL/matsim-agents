...
init_run_dirs "$PROJ" "${SERVE_RUN_PREFIX:-vllm-multinode}" "${SLURM_JOB_ID:-$$}"
...
GPUS_PER_NODE=4
N_NODES=${SERVE_N_NODES:-${SLURM_JOB_NUM_NODES:-${SLURM_NNODES:-1}}}
...
wait_for_ray_gpus() {
  local expected=$1 max_wait=$2 monitor_pid=${3:-} total=0
  local started=$SECONDS deadline=$(( SECONDS + max_wait ))
  while (( SECONDS < deadline )); do
    if [[ -n "$monitor_pid" ]] && ! kill -0 "$monitor_pid" 2>/dev/null; then
      wait "$monitor_pid" 2>/dev/null || true
      echo "[ray] Head process exited before registering $expected GPUs." >&2
      return 1
    fi
    total=$(ray_gpu_count | tail -1)
    [[ "$total" =~ ^[0-9]+$ ]] || total=0
    if (( total >= expected )); then
      echo "[ray] $total/$expected GPUs available after $(( SECONDS - started ))s."
      return 0
    fi
    sleep 5
  done
...
if [[ "$N_NODES" -gt 1 ]]; then
  ...
  mapfile -t ALL_NODES < <(scontrol show hostnames "${SERVE_NODELIST:-$SLURM_JOB_NODELIST}")
  ...
  # Keep this root deliberately short: Ray appends a timestamped session path,
  # and Linux AF_UNIX socket paths may not exceed 107 bytes. The Slurm step ID
  # distinguishes concurrent model clusters within a coordinated allocation.
  RAY_TMPDIR="/tmp/r${SLURM_JOB_ID:-$$}.${SLURM_STEP_ID:-0}"
  for attempt in 1 2 3; do
    echo "[ray] Starting head node at $RAY_ADDRESS (attempt $attempt/3) ..."
    rm -rf "$RAY_TMPDIR"
    ...
    if wait_for_ray_gpus "$GPUS_PER_NODE" 180 "$RAY_HEAD_PID"; then
      break
    fi
    kill "$RAY_HEAD_PID" 2>/dev/null || true
    wait "$RAY_HEAD_PID" 2>/dev/null || true
    timeout 30s "$VLLM_VENV/bin/ray" stop --force >/dev/null 2>&1 || true
    RAY_HEAD_PID=""
    (( attempt < 3 )) || { echo "[ray] ERROR: head failed after 3 attempts." >&2; exit 1; }
    sleep 30
  done
