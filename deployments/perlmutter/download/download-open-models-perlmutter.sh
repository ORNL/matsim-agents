...
if ! python -c 'import huggingface_hub' >/dev/null 2>&1; then
  echo "ERROR: huggingface_hub not found in active environment." >&2
...
for model_id in "${MODELS[@]}"; do
  ...
  mkdir -p "$dest"
  echo
  echo "[$(date)] Downloading $model_id -> $dest (direct, atomic files)"

  if python "$REPO/deployments/common/download_hf_snapshot.py" "$model_id" "$dest" >"$log" 2>&1; then
