...
load_perlmutter_nvhpc_modules() {
    ...
    local stack="${REPO}/deployments/perlmutter/setup/perlmutter-module-stack.sh"
...
write_makefile_include() {
    ...
    if [[ -n "${SCALAPACK_ROOT}" ]]; then
        ...
        if [[ -n "${sca_lib_path}" ]]; then
            ...
            warn "  build it first: bash deployments/perlmutter/setup/build-scalapack-perlmutter.sh"
        fi
    else
...
if [[ -n "${SCALAPACK_ROOT}" && "${SCALAPACK_AUTOBUILD}" == "1" ]]; then
    if [[ ! -f "${SCALAPACK_ROOT}/lib/libscalapack.a" \
       && ! -f "${SCALAPACK_ROOT}/lib64/libscalapack.a" ]]; then
        ...
        bash "${REPO}/deployments/perlmutter/setup/build-scalapack-perlmutter.sh" \
