#!/usr/bin/env bash
# One-shot, self-contained matsim-agents + HydraGNN environment for NERSC Perlmutter.
set -Eeuo pipefail
...
INSTALL_ROOT="${INSTALL_ROOT:-${HYDRAGNN_DIR}/installation_DOE_supercomputers/HydraGNN-Installation-Perlmutter}"
...
die() { printf '\033[1;31m[perlmutter-install]\033[0m %s\n' "$*" >&2; exit 1; }
...
HYDRAGNN_INSTALLER="${HYDRAGNN_DIR}/scripts/hpc/nersc/perlmutter/installation/install.sh"
[[ -f "${HYDRAGNN_INSTALLER}" ]] || die "HydraGNN Perlmutter installer not found: ${HYDRAGNN_INSTALLER}"

log "Installing HydraGNN and its CUDA/PyG/MPI dependencies first"
INSTALL_ROOT="${INSTALL_ROOT}" VENV_PATH="${VENV_PATH}" EXPECTED_CUDA_MM="${EXPECTED_CUDA_MM}" \
