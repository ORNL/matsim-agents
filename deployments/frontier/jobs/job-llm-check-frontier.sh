...
#SBATCH -q debug
# ---------------------------------------------------------------------------
# Qualify one locally stored model through a live vLLM server on Frontier
# (AMD MI250X, ROCm 7.2).
#
...
# portability run, and cleanup. Mirrors
# deployments/perlmutter/jobs/job-llm-check-perlmutter.sh, adapted for the
# ROCm 7.2 module/venv stack (see deployments/frontier/setup/frontier-module-stack.sh
# and deployments/frontier/jobs/job-serve-multinode-frontier.sh for the vLLM
# env-var conventions this reuses).
#
...
#   MATSIM_VLLM_TP             tensor-parallel/GCD count (default: 8)
...
#     deployments/frontier/jobs/job-llm-check-frontier.sh
# ---------------------------------------------------------------------------
...
[[ -x "${VENV}/bin/vllm" ]] || { echo "ERROR: vLLM executable not found in ${VENV}" >&2; exit 2; }
mkdir -p "${CHECK_ROOT}"

echo "[$(date)] Starting ${MODEL_NAME} from ${MODEL_DIR} (tp=${VLLM_TP}, 8 GCDs/node on MI250X)"
(
  ...
  # Use prebuilt tvm_ffi torch-c-dlpack .so from proj-shared (avoids JIT rebuild
  # hang at import) — same convention as job-serve-multinode-frontier.sh.
  ...
  export MIOPEN_USER_DB_PATH="${JIT_TMP}/miopen"
...
# Load the same environment for the client. It communicates through HTTP and
# does not import the server's torch stack, but re-uses the venv/module stack
# already active for consistency with the rest of the Frontier job scripts.
source "${REPO}/deployments/frontier/setup/frontier-module-stack.sh"
load_frontier_rocm72_modules
source /sw/frontier/miniforge3/23.11.0-0/etc/profile.d/conda.sh
source activate "${VENV}"
...
CHECK_RUN="${CHECK_RUN}" "${VENV}/bin/python3" -c \
...
if [[ "${MATSIM_RUN_PORTABILITY:-0}" == "1" ]]; then
  ...
  "${VENV}/bin/python3" benchmarks/portability/run.py \
    --facility frontier \
    --suite all \
    --backend qe \
    --execute \
