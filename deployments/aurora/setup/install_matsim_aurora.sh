...
# installed by the editable `pip_retry -e "${MATSIM_DIR}[...]"` step above.
# Modern pyxtal (>=1.0) is a pure-Python wheel with no Fortran/pyshtools build
# step, so no separate/non-fatal install step is needed here.
